...
const ScrollableCard = ({ productList, navigation }) => {
  const renderImage = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => {
        navigation.navigate('ItemDetails', { item: 'hello' });
      }}>
      <Image
        source={item.image}
        resizeMode="cover"
        style={{ backgroundColor: 'red', width: 200, height: 500 }}
      />
      <View
        style={{
          position: 'absolute',
...
const ScrollableTab = ({ tabList1, selectedTab1, onPress }) => {
  ...
  const renderItem = ({ item }) => (
    ...
    <TouchableOpacity
      ...
      {/* Very Intersting here == selectedTab1 idla 0 iruntha athai thaan item idlaium irukkum item 0*/}
...
const Home = ({ navigation }) => {
  ...
  const [selectedTab, setSelectedTab] = React.useState({
    ...
    productList: [
      {
        productId: 1,
        productName: 'Chair Green Colour1',
        price: 10.0,
        image: images.greenChair,
      },
      {
        productId: 2,
        productName: 'Chair Peach Colour1',
        price: 10.0,
        image: images.redChair,
      },
      {
        productId: 3,
        productName: 'Chair White Colour1',
        price: 10.0,
        image: images.whiteChair,
      },
    ],
